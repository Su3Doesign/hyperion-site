...
- acts.js v5
- Fixes:
- - Act 3 dashboard target derived from steering (screen mesh origin is broken)
- - Act 3 dwell holds a usable dashboard composition
- - Act 1 adds engine dwell — camera slows near peak engine reveal
- - Act 4 pulled back and elevated so the dawn scene breathes around a smaller car
- - Act 5 headlight fade restored to the old “muah chef’s kiss” version
    */
...
// Use steering as the reliable interior anchor — screen mesh origin is at car origin (broken)
...
// Proper dashboard target — derived from steering position, offset forward
// Steering is at (0.45, 0.64, -0.31). Dashboard is ~ 0.4m FORWARD of the steering wheel.
const dashboardTarget = interiorAnchor
? new THREE.Vector3(
interiorAnchor.x + front.x * 0.35,   // forward from steering
interiorAnchor.y + 0.05,              // slightly up — screen is at top of dash
interiorAnchor.z + front.z * 0.35
)
: tgt(0.3, 0, H * 0.5);

// Driver-head position — behind steering wheel
...
// ACT 3 — Dashboard composition (camera OUTSIDE driver window, looking at dash)
{ pos: pos(LEN * 0.2, WID * 1.2, H * 0.55),
  tgt: dashboardTarget.clone() },

// ACT 4 — PULLED WAY BACK + HIGHER for atmospheric dawn scene
// Car is small in frame, light dominates, space to breathe
...
// ============================================
// CAMERA — with DWELLS on Act 1 (engine) and Act 3 (dashboard)
// ============================================
...
// ACT 1 DWELL — slow approach to engine at peak reveal (50-70% of act)
if (i === 1) {
if (tRaw < 0.5) {
t = tRaw * 0.7;                               // ease-in toward engine
} else if (tRaw < 0.75) {
t = 0.35 + (tRaw - 0.5) * 0.2;                // near-stall at engine
} else {
t = 0.4 + ((tRaw - 0.75) / 0.25) * 0.6;       // roll out toward Act 2
}
}

// ACT 3 DWELL — hold on dashboard 35-65%
if (i === 3) {
if (tRaw < 0.35) {
t = (tRaw / 0.35) * 0.5;
} else if (tRaw < 0.65) {
t = 0.5;                                      // full hold on dashboard
} else {
t = 0.5 + ((tRaw - 0.65) / 0.35) * 0.5;
}
}
...
// Restored — the “muah chef’s kiss” version
